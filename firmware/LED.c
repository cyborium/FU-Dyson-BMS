                     ...
                     int8_t pwm_fade_slope)
{
    ...
    { //starting blank time
        step++;
        if (num_blinks != 0)
        { // Leave LED off if num_blinks is zero. Next step will be cycle complete.
    ...
    { //Fade out
        ...
        else if (current_pwm <= -pwm_fade_slope)
        {
            ...
            if (num_blinks == 1)
            { //If there is only one blink in the cycle, and the PWM fade is done, do all the clean up procedures immediately to avoid brief LED blank time
                nonblocking_wait_counter.enable = 0;
                nonblocking_wait_counter.value = 0;
                if (LED_code_cycle_counter.enable)
                {
