...
void ISL_Init(void)
{
    ...
    ISL_SetSpecificBits((uint8_t[])
    {
        WriteEnable, 5, 3}, 0b111); //Set all three feature set, charge set, and discharge set write bits
    ...
    ISL_SetSpecificBits((uint8_t[])
    {
        WriteEnable, 5, 3}, 0b111); //We likely need to enable register writes again
    ...
    ISL_SetSpecificBits((uint8_t[])
    {
        WriteEnable, 5, 3}, 0b000); //Clear all three feature set, charge set, and discharge set write bits
...
{ //Allows easily retrieving an entire register. Ex. ISL_Read_Register(ISL_CONFIG_REG); result = ISL_RegData[Config]
...
{ //Can be used like "ISL_GetSpecificBits_cached(ISL.WKUP_STATUS, ISL_RegData[status])
    uint8_t reg_addr = params[REG_ADDRESS];
    uint8_t bit_addr = params[BIT_ADDRESS];
    uint8_t bit_length = params[BIT_LENGTH];
...
void ISL_calcCellStats(void)
{
    ...
    { //We can start with cell 2 since we already assumed cell 1 is max/min until proved otherwise.
...
static uint8_t _GenerateMask(uint8_t length)
{ //Generates a given number of ones in binary. Ex. input 5 = output 0b11111
    uint8_t result = 0b1;
    for (; length > 1; length--)
    {
