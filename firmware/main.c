...
void ClearI2CBus()
{
    ...
    while (validOnes < 10)
    {
        ...
        { //Read data and check if SDA is high (idle). Also make sure SCL isn't getting glitched low
            validOnes++;
        }
        else
        {
...
uint16_t static ConvertADCtoMV(uint16_t adcval)
{ //I included this function here and in isl94208 so that it could stand alone if needed. There is probably a better way to do this.
...
{ //Adds routine to switch to DAC VSS output to clear sample/hold capacitor before taking real reading
    ADCPrepare();
...
void init(void)
{
    ...
    { //If bus is still not idle (meaning pins aren't high), which shouldn't be possible, then keep trying to clear bus. Do not pass go. Do not collect $200.
        //    __debug_break();
        ClearI2CBus();
    }
...
void idle(void)
{
    ...
    if (detect == TRIGGER //Trigger is pulled
            ...
            && !full_discharge_flag //Make sure pack hasn't just been fully discharged
            && safetyChecks())
    {
        state = OUTPUT_EN;
    }
    else if (detect == TRIGGER && full_discharge_flag)
    {
        state = ERROR;
    }
    else if (detect == CHARGER //Charger is connected
            && !charge_complete_flag //We haven't already done a complete charge cycle
            && maxCellOK() //Max cell < 4.20V
            && ISL_GetSpecificBits_cached(ISL.WKUP_STATUS) //Make sure WKUP = 1 meaning charger connected or trigger pressed
            && safetyChecks())
    {
        if ((show_cell_delta_LEDs == 1 && cellDeltaLEDIndicator()) || !show_cell_delta_LEDs)
        {
            state = CHARGING;
        }
    }
...
#endif
            && !sleep_timeout_counter.enable && safetyChecks())
    {
    ...
    { //Pack on charger and charge is complete = Green LED
        Set_LED_RGB(0b010, 1023); //LED Green
    }
    else if (detect == CHARGER && ISL_GetSpecificBits_cached(ISL.WKUP_STATUS))
    { //Pack on charger but charge isn't complete and we aren't in a charging or error state = Yellow LED, undefined state
        Set_LED_RGB(0b110, 1023); //LED Yellow
    }
    else if (detect == NONE)
    {
        if (CheckStateInDetectHistory(CHARGER))
        { //Show cell delta code when charger removed after complete charge
            previous_detect_was_charger = true; //This flag is set if we are transitioning from detect == CHARGER to detect == NONE
        }

        if (cellDeltaLEDIndicator() || !previous_detect_was_charger)
    ...
    { //trigger is pulled but we didn't enable output, and it isn't because there was an error or the pack is fully discharged = Yellow LED
    ...
    { //If the max cell voltage is below 4100mV and the pack is marked as fully charged, unmark it as charged.
        charge_complete_flag = false;
        show_cell_delta_LEDs = 0;
    }

    if (detect != CHARGER)
    { //Also, if removed from charger, clear charge complete flag.
        charge_complete_flag = false;
        if (!full_discharge_flag && !minCellOK())
        { //If min cell voltage is too low and we aren't on the charger, set full_discharge_flag so we can reference it for LED codes.
            full_discharge_flag = true;
        }
    }

    if (sleep_timeout_counter.value > IDLE_SLEEP_TIMEOUT && sleep_timeout_counter.enable)
    { //938*32ms = 30.016s //If we are in IDLE state for 30 seconds and not on the charger, go to sleep. We will stay awake on the charger since we have power to spare and can then make sure battery voltages don't drop over time.
...
void charging(void)
{
    ...
    { //Target voltage reached
        ...
        { //Go to charge wait state and wait 70 seconds before starting next charge cycle
            state = CHARGING_WAIT;
        }
    }
    ...
    { //charger removed before complete charge
        ISL_SetSpecificBits(ISL.ENABLE_CHARGE_FET, 0); //Disable Charging
        charge_duration_counter.enable = false; //Stop charge timer
        state = IDLE;
...
{
    if (detect == CHARGER)
    { //Don't set the LED like this if the charger isn't connected because it will interfere with the cellDeltaLEDIndicator
        Set_LED_RGB(0b111, 1023); //White LED
    }

    if (!charge_wait_counter.enable)
    { //if counter isn't enabled, clear and enable it.
...
void outputEN(void)
{
    ...
    { //If we hit the min cell voltage cut off, prevent discharging battery further until it is put on charger
        full_discharge_flag = true;
    ...
    { //Charger attached while trigger was pulled
        ISL_SetSpecificBits(ISL.ENABLE_DISCHARGE_FET, 0); //Disable discharging
        need_to_clear_LEDs_for_cell_voltage_indicator = 1;
        ...
        if (LED_code_cycle_counter.value > 1)
        { //One LED cycle completed
            state = IDLE;
        }
    }
    else
    { //Trigger released; WKUP status = 1
        ISL_SetSpecificBits(ISL.ENABLE_DISCHARGE_FET, 0); //Disable discharging
        runonce = 0;
        if (need_to_clear_LEDs_for_cell_voltage_indicator == 1)
        { //Prevents weird things from happening if you release the trigger in the middle of the startup LED sequence
            resetLEDBlinkPattern();
            need_to_clear_LEDs_for_cell_voltage_indicator = 0;
        }
        if (cellVoltageLEDIndicator())
        { //Repeat the cellVoltageLEDIndicator() function until one iteration, when it will return true.
            state = IDLE;
        }
    }
...
void error(void)
{
    ...
    { // In case normal Output enable clean up was bypassed due to brown out or I2C error
        total_runtime_counter.enable = false;
        //Write latest runtime counter value to EEPROM
        WriteTotalRuntimeCounterToEEPROM(EEPROM_RUNTIME_TOTAL_STARTING_ADDR);
    }
    ...
    { //full_discharge_trigger_error isn't an actual error that needs recording
        //Assemble first byte
        uint8_t data_byte_1 = past_error_reason.ISL_INT_OVERTEMP_FLAG << 7; //Due to bit-field used in declaration, these can only be one bit.
        ...
        uint8_t data_byte_2 = past_error_reason.DISCHARGE_OC_SHUNT_PICREAD << 7;
        ...
        if (!(((uint16_t) starting_write_addr + byte_size_of_event_log + byte_size_of_event_log - 1) > 255))
        { //Make sure that there is enough room left after we write our 6 bytes. Subtract one because we are checking if the ending of the next event would be out of bounds.
    ...
    if (critical_i2c_error || past_error_reason.ISL_BROWN_OUT)
    {
        ...
        { //It's called critical for a reason
            ...
            if (TMR4_HasOverflowOccured() && nonblocking_wait_counter.enable)
            { //Every 32ms //Since we aren't going in to main loop again, we still have to service this counter for the LED code to work
            ...
            && discharge_current_mA == 0)
    {
        ...
        else if (error_timeout_wait_counter.value > ERROR_EXIT_TIMEOUT
    ...
    if (detect == CHARGER)
    {//If at any point the charger is connected, abort sleep sequence. I'm not quite sure why we'd want to though.
        sleep_timeout_counter.enable = false;
    }
    else if (sleep_timeout_counter.enable == false)
    { //If there is an error, start sleep counter (if it isn't already started), so we sleep if in error state for too long
        sleep_timeout_counter.value = 0;
        sleep_timeout_counter.enable = true;
    }
...
{ //Make sure there are four bytes available with the provided starting address
...
void main(void)
{
    ...
    while (1)
    {
        ...
        { //If we were successful this time, clear the error counter.
            I2C_error_counter = 0;
        }
