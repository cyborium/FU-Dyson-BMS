...
{ //Binary search algorithm with minor tweaks and rounding
    ...
    while (!(ThermistorLUT[modelnum][index][voltage] * 10 <= pic_thermistor) || !(ThermistorLUT[modelnum][index + 1][voltage] * 10 > pic_thermistor))
    {
        ...
        if (index + increment > LUT_SIZE[modelnum] - 1 || index - increment < 0)
        { //if the next increment step would go out of bounds, just break. Subtract one because arrays are zero indexed
            break;
        }
    ...
    if (index == LUT_SIZE[modelnum] - 1)
    { //If the resulting index is the max value, reduce it by one so our distance to i calcs work and i+1 doesn't go out of bounds. Arrays are zero indexed.
        index = index - 1;
    }
    ...
    if (dist_to_i < dist_to_i_plus_one)
    { //If pic_thermistor is closer to i, return i
